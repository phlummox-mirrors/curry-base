...
mkSingleLet e2 e1 [v] = return (Let [(v, e1)] e2)
mkSingleLet body bound fbs = do
  recname <- newLocalName (Just fbsType)
...
nonrecLet x e1 e2
  | x `elem` allVars e1 = do vi <- newLocalName (typeofVar x)
...
mkTupleConstr arity = curry mkQName "Prelude"
  ('(' : replicate (length arity - 1) ',' ++ ")")
