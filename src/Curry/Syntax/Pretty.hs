...
module Curry.Syntax.Pretty
  ...
  , ppExpr, ppOp, ppStmt, ppFieldExpr, ppTypeExpr, ppAlt, ppContext, ppQIdent
  ) where
...
ppExpr p (Typed  _ e cx ty) =
  parenIf (p > 0) (ppExpr 0 e <+> text "::" <+> ppContext cx <+> ppTypeExpr 0 ty)
...
ppExpr _ (EnumFromThenTo _ e1 e2 e3) =
  brackets (ppExpr 0 e1 <> comma <+> ppExpr 0 e2
              <+> text ".." <+> ppExpr 0 e3)
ppExpr p (UnaryMinus _     op e) = parenIf (p > 1) (ppInfixOp op <> ppExpr 1 e)
