...
instance Equiv IDecl where
  ...
  IFunctionDecl _ f1 n1 c1 ty1 =~= IFunctionDecl _ f2 n2 c2 ty2
    = f1 == f2 && n1 == n2 && c1 == c2 && ty1 == ty2
  _ =~= _ = False
...
instance FixInterface IDecl where
  fix tcs (IDataDecl     p tc tvs     cs) =
    IDataDecl p tc tvs (fix tcs cs)
  fix tcs (INewtypeDecl  p tc tvs     nc) =
    INewtypeDecl  p tc tvs (fix tcs nc)
  fix tcs (ITypeDecl     p tc tvs     ty) =
    ITypeDecl p tc tvs (fix tcs ty)
  fix tcs (IFunctionDecl p f  n   con ty) =
    IFunctionDecl p f  n con (fix tcs ty)
  fix _   d                               = d
